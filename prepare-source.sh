...
readonly PATCHED_DOCKERFILE_SHA256='6e0e2589cf55414918a9186c0fe8644e9e5f04a53fabb4dbd1510db3bdf97ab9'
...
readonly SECURITY_PATCH_SHA256='d7273a55464531aa39361d7bc64a6a069f544fef9e2bb4d8b12367de7517aa51'
...
assert "FROM uv_source AS sqlite_build" in dockerfile
assert dockerfile.count("FROM uv_source") == 2
assert "FROM debian:" not in dockerfile
assert dockerfile.count("https://snapshot.debian.org/archive/debian/20260925T000000Z") == 2
assert dockerfile.count("https://snapshot.debian.org/archive/debian-security/20260925T000000Z") == 2
assert dockerfile.count("714d457d580922dbf1d0be8bd35ba236a842b50b0072ae791582a19adef772a5") == 2
assert "rm -rf /root/.cache/uv /usr/local/lib/python3.13/site-packages" in dockerfile
print("security_patch=vitest-4.1.11,anyio-4.14.2,h2-4.4.1,hpack-4.2.0,httpx2-httpcore2-2.12.0,pinned-runtime-bases,debian-snapshot-20260925T000000Z,verified-ca-bootstrap,clean-runtime-python")
PY
